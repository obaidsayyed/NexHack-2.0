...
import { sanitizeName, sanitizeNotes } from '../utils/inputValidation';
...
export const Patients: React.FC<PatientsProps> = ({
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!regFirstName.trim() || !regLastName.trim()) {
      setErrorMsg('First name and last name are required and may contain letters, spaces, apostrophes, or hyphens only.');
      return;
    }

    if (!Number.isInteger(regAge) || regAge < 18 || regAge > 115) {
      setErrorMsg('Age must be a whole number between 18 and 115 years.');
      return;
    }

    if (regNotes.trim().length > 1000) {
      setErrorMsg('Observation notes must be 1000 characters or fewer.');
      return;
    }
        ...
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) => { const value = e.target.value; setRegAge(value === '' ? 0 : Math.max(0, Number(value))); }}
            />
            <div>
          ...
          <div>
            ...
            <textarea
              rows={3}
              value={regNotes}
              onChange={(e) => setRegNotes(sanitizeNotes(e.target.value))}
