...
import { positiveNumberInput } from '../../utils/inputValidation';
...
export const PredictionForm: React.FC<PredictionFormProps> = ({
  ...
  const handleTextNumberChange = (field: keyof ClinicalFeatures, value: string) => {
    const sanitized = positiveNumberInput(value).replace(/e/gi, '').replace(/\+/g, '');
  ...
  const validateForm = (): boolean => {
    ...
    const numericRules: Record<string, { min: number; max: number; label: string; integer?: boolean }> = {
      Age: { integer: true, min: 18, max: 115, label: 'Age' },
      BMI: { min: 10, max: 75, label: 'BMI' },
      Previous_HF_Admissions: { integer: true, min: 0, max: 50, label: 'Previous HF admissions' },
      Previous_Hospital_Admissions: { integer: true, min: 0, max: 100, label: 'Previous hospital admissions' },
      Ejection_Fraction: { integer: true, min: 5, max: 85, label: 'Ejection fraction' },
      Systolic_BP: { integer: true, min: 60, max: 250, label: 'Systolic BP' },
      Diastolic_BP: { integer: true, min: 30, max: 150, label: 'Diastolic BP' },
      Heart_Rate: { integer: true, min: 30, max: 220, label: 'Heart rate' },
      Oxygen_Saturation: { integer: true, min: 50, max: 100, label: 'Oxygen saturation' },
      Creatinine: { min: 0.2, max: 20, label: 'Creatinine' },
      Sodium: { integer: true, min: 100, max: 180, label: 'Sodium' },
      Potassium: { min: 2, max: 10, label: 'Potassium' },
      Hemoglobin: { min: 3, max: 25, label: 'Hemoglobin' },
      Blood_Glucose: { integer: true, min: 20, max: 1000, label: 'Blood glucose' },
      BNP: { integer: true, min: 0, max: 25000, label: 'BNP' },
      Length_of_Stay: { integer: true, min: 0, max: 90, label: 'Length of stay' },
    };

    Object.entries(numericRules).forEach(([field, rule]) => {
      const value = formData[field as keyof ClinicalFeatures] as unknown as number;
      if (!Number.isFinite(value)) {
        errors[field] = `${rule.label} is required and must be a number.`;
  ...
  const fieldConstraints: Partial<Record<keyof ClinicalFeatures, { min: number; max: number; step: number }>> = {
    Age: { min: 18, max: 115, step: 1 },
    BMI: { min: 10, max: 75, step: 0.1 },
    Previous_HF_Admissions: { min: 0, max: 50, step: 1 },
    Previous_Hospital_Admissions: { min: 0, max: 100, step: 1 },
    Ejection_Fraction: { min: 5, max: 85, step: 1 },
    Systolic_BP: { min: 60, max: 250, step: 1 },
    Diastolic_BP: { min: 30, max: 150, step: 1 },
    Heart_Rate: { min: 30, max: 220, step: 1 },
    Oxygen_Saturation: { min: 50, max: 100, step: 1 },
    Creatinine: { min: 0.2, max: 20, step: 0.1 },
    Sodium: { min: 100, max: 180, step: 1 },
    Potassium: { min: 2, max: 10, step: 0.1 },
    Hemoglobin: { min: 3, max: 25, step: 0.1 },
    Blood_Glucose: { min: 20, max: 1000, step: 1 },
    BNP: { min: 0, max: 25000, step: 1 },
    Length_of_Stay: { min: 0, max: 90, step: 1 },
  };
      ...
      <div className="flex flex-col">
        ...
        {type === 'select' ? (
          ...
          <input
            ...
            step={fieldConstraints[field]?.step ?? '1'}
            value={formData[field] as number | string}
            onKeyDown={(e) => {
